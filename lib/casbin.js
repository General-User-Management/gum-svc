...
async function _newEnforcerInstance (model, adapter, watcher) {
...
async function Casbin () {
  ...
  let e = await _newEnforcerInstance(model, adapter, watcher)
  global.enforcer = e

  watcher.setUpdateCallback(async () => {
    console.log('Casbin need update,reload Policy~~')
    e = await _newEnforcerInstance(model, adapter)
    global.enforcer = e
  })
}
